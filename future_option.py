...
def to_excel():
    ...
    df.insert(10, "外資SC口數", df["外資BC口數"] - df["外資CALL口數"])
    df.insert(11, "外資SC金額", df["外資BC金額"] - df["外資CALL金額"])
    df.insert(12, "外資BC金額與結算差", df["外資BC金額"])
    df.insert(13, "外資SC金額與結算差", df["外資SC金額"])
    df.insert(18, "外資SP口數", df["外資BP口數"] - df["外資PUT口數"])
...
def option_price(date):
    ...
    call["換算台指"] = call["履約價"] + call["最後成交價"] - call["最後成交價"].shift(-1)
    ...
    put["換算台指"] = put["履約價"] - put["最後成交價"] + put["最後成交價"].shift(1)
